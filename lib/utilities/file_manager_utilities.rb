...
module FileManager
  ...
  def self.alive? session_id, serverport = SYSTEM_DEFAULT_SPIN_SERVER_PORT, server_name = SYSTEM_DEFAULT_SPIN_SERVER
    # test if the file manager is alive or not
    my_server_params = Spinapp::SpinFileServer.find_by_server_port serverport
    ...
    if ENV['RAILS_ENV'] == 'development'
      uri_str = protocol + '://' + host + ':' + port.to_s + '/filemanager/system/is_alive.tdx'

      log_msg = 'Rails : request is_alive. URI = ' + uri_str
      rails_logger(log_msg)
    ...
    return true if resp.code == 200 # => is OK
  ...
  def self.busy? serverport = SYSTEM_DEFAULT_SPIN_SERVER_PORT,server_name = SYSTEM_DEFAULT_SPIN_SERVER
    my_server_params = Spinapp::SpinFileServer.find_by_server_port serverport
    max_connections = my_server_params[:max_connections]
    max_connections = -1 if max_connections.nil?

    begin
      Spinapp::SpinProcess.transaction do
        #        Spinapp::SpinProcess.find_by_sql('LOCK TABLE spin_processes IN EXCLUSIVE MODE;')
        active_procs = Spinapp::SpinProcess.readonly.where(['( proc_action = 1 OR proc_action = 2 ) AND proc_status = 1'])
        return true if active_procs.size > (max_connections != -1 ? max_connections : MAX_CONCURRENT_FILEMANAGER_PROCS)
      ...
      rails_logger 'no active spin spocesses in table'
      false
    end

    #    begin
    #      all_procs = Spinapp::SpinProcess.readonly.where(["id > 0"])
    #      if all_procs.size > 0
    #        return true
    #      else
    #        return false
    ...
    #    end
    #    # test if the file manager is alive or not
    #    my_server_params = Spinapp::SpinFileServer.find_by_server_name server_name
    #    host = my_server_params[:server_host_name]
    #    port = my_server_params[:server_port]
  ...
  def self.logger sid, log_msg, serverport = SYSTEM_DEFAULT_SPIN_SERVER_PORT,server_name = SYSTEM_DEFAULT_SPIN_SERVER, log_level = LOG_INFO
    # buid request params hash
    ret = true
    if server_name == 'LOCAL'
      rails_logger log_msg, log_level
    else
      request_params = { session_id: sid, request: 'put_log', params: [ log_msg ] }
      server_params = Spinapp::SpinFileServer.find_by_server_port serverport
      #      server_params = Spinapp::SpinFileServer.find_by_server_name 'spinlogserver-local'
      ret = post_request request_params, server_params
    end
    ret
    #    return true
  end # => end of self.logger sid, log_msg, server_name = '127.0.0.1:18880'

  def self.rails_logger message = "#{self.class.name}.#{__method__}", log_level = LOG_INFO
    case log_level
    when LOG_ERROR
      t = Time.now
  ...
  def self.request_remove_node sid, node_key, server_params = nil
    ...
    ret = post_request request_params, server_params
    ret
  end # => end of self.request_remove_node sid, node_key, server_params = nil
  ...
  def self.request_trash_node sid, node_key, server_params = nil
    ...
    ret = post_request request_params, server_params
    ret
  end # => end of self.request_trash_node sid, node_key, server_params = nil
  ...
  def self.post_request request_params, server_params = nil
    ...
    if server_params.present? # => server params are passed
      resp = my_http_post server_params, request_params
    else # => use default
      if $my_sever_params_g.present?
        my_server_params = $my_server_params_g
      else
        my_server_params = Spinapp::SpinFileServer.find_by_server_port SYSTEM_DEFAULT_SPIN_SERVER_PORT
        $my_server_params_g = my_server_params
      end
      resp = my_http_post my_server_params, request_params
    end
    resp
  end # => end of self.post_request request_params, server_params
  ...
  def self.my_http_post(server_params, request_params)
    ...
    if ENV['RAILS_ENV'] == 'development'
      ...
      my_server_port = 18_881
    else
      my_server_name = server_params[:server_host_name]
      my_server_port = server_params[:server_port].to_i
    end
...
class OldFileManagerClient
  ...
  def initialize(protocol,host,port,location,user,passwd)
    ...
    raise res[:MVFS_ERROR_MSG] if res.key?(:MVFS_ERROR_MSG)
  ...
  def tree_folder_recursive(folder)
    ...
    if /MVFS_ERROR_MSG=(.+)/m =~ body
      res.store(:MVFS_ERROR_MSG, $1)
    elsif /MVFS_FOLDERS_NUMBER=(.+)/m =~ body
      fn = $1
      fn_array = fn.split("\n")
  ...
  def _folder_tree(folder, count, fn_array)
    ...
    fn_array.each do |f|
      ...
      if $2.to_i == 0
        ...
        if n > 0
          n.downto(1) do |i|
            next unless counter_by_level[i][0] <= counter_by_level[i][1]
  ...
  def list_file_path(folder)
    item_names = %w[FilePath_ FileName_].join("\x1e")
    body = http_post('MemberListFiles', {
        SessionID_: @session_id,
        ItemNames: item_names,
        FolderPath: folder,
  ...
  def get_folder_attributes(folder)
    item_names = %w[FolderName_ SubFolders_ SubFiles_ SessionTimeout_ Message_
  ...
  def set_file_attributes(file_path,attributes_hash)
    ...
    attributes_hash.each do |k,v|
      attributes = attributes + "\x1e" if attributes != ''
  ...
  def group_member_list(group_id_code)
    ...
    if /MVFS_ERROR_MSG=(.+)/m =~ body
      res.store(:MVFS_ERROR_MSG, $1)
    elsif /MVFS_GROUP_MEMBER_LIST=(.+)/m =~ body
      member = []
      $1.split("\r\n").each do |m|
        if /(\d+)\.\[(.+)\](.+)/ =~ m
          member.push([$1,$2,$3])
  ...
  def search_file(opt)
    ...
    param[:FileName_] = param[:FileName_] if param.key?(:FileName_)
  ...
  def get_file(file, version=nil)
    ...
    param[:Version] = version unless version.nil?
