        ...
        case event_type
          ...
          when 'selected_domain'
            ...
            if selected_folder.present?
              ...
              rethash[:success] = false
              rethash[:status] = ERROR_FAILED_TO_UPDATE_FOLDER_LIST
              rethash[:errors] = 'failed to select folder : update folder list'
            end
          else # => another events
        ...
        retry_encrypt = ENCRYPTION_RETRY_COUNT
          ...
          begin
            ...
            if ENV['RAILS_ENV'] != 'production'
              ...
              # => get URL server
              my_url_host = SpinFileServer.find_by_server_name('127.0.0.1:18881')
              if my_url_host.present?
                my_host = my_url_host[:spin_url_server_name]
              end
              #          if my_host.length == 2
              rethash[:redirect_uri] = my_host + "/filemanager/downloader/download_proc?fmargs=" + fmargs
            else
        ...
        retry_encrypt = ENCRYPTION_RETRY_COUNT
          ...
          begin
            ...
            if ENV['RAILS_ENV'] != 'production'
              ...
              #          if my_host.length == 2
              rethash[:redirect_uri] = my_host + "/filemanager/uploader/upload_proc?fmargs=" + fmargs
            else
